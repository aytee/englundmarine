...
class EnglundProducts
{
	...
	function getProductTypeList()
	{
		...
		$query = 'SELECT dw_item.dwin_display_item_number AS item_number, view_representative_items.dwin_display_item_number AS cluster,
      ...
      LEFT JOIN dw_item AS dw_item_1 ON view_representative_items.dwin_item_number = dw_item_1.dwin_item_number
     	LEFT JOIN `view_dw_department` ON `dw_item`.`dwin_store`=`view_dw_department`.`dwde_store_number` AND `dw_item`.`dwin_department`=`view_dw_department`.`dwde_department` 

      ';


		if($this->department !=''){
			$query .= 'WHERE view_dw_department.dwde_department = %s';
		}
//print_r($query);
